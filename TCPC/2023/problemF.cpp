...
int sub(T u, U v, int mod = MOD) {
    return ((u % mod) + 0ll - (v % mod) + 2 * mod) % mod;
}
template <typename T>
...
int starsandbars(int n, int k, int mod = MOD) { return cnk(n + k - 1, n); }

int n, m;
void test_case() {
    cin >> n >> m;
    int invm = inv(m);
    int tot = sub(powe(2, n), 1);
    int lsum = mul(tot, invm);
    int rsum = 0;
    int po = 1;
    for (int i = 1; i <= n; i++) {
        int rem = m - i;
        ...
        po = (po * 1ll * invm) % MOD;
        rsum = (rsum +
                (((starsandbars(rem, i) * 1ll * cnk(n, i)) % MOD) * 1ll * po) %
                    MOD) %
               MOD;
    }
    int ans = sub(lsum, rsum);
    ans = (ans + 1) % MOD;
    cout << ans << endl;
}
