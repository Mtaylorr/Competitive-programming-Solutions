...
class segtree {
   public:
    ...
    int getFirst(int qs, int qe, int id, int ns, int ne) {
        if (qs > ne || qe < ns) return -1;
        if (tree[id].mn == 1) return -1;
        if (ns == ne) return ns;
        int l = left(id);
        int r = right(id);
        int md = mid(ns, ne);
        int p = getFirst(qs, qe, l, ns, md);
        if (p == -1) {
            p = getFirst(qs, qe, r, md + 1, ne);
        }
        return p;
    }
    ...
    int getFirst(int qs, int qe) {
        assert(0 <= qs && qs <= qe && qe <= n - 1);
        return getFirst(qs, qe, 0, 0, n - 1);
    }
    node get(int p) {
        assert(0 <= p && p <= n - 1);
